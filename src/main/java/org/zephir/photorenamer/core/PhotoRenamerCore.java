...
public class PhotoRenamerCore {
  ...
  public void processFolder() throws CustomException {
    ...
    log.debug("{} photo(s) renamed ({} rotated) and {} video(s) renamed in {}ms",
        nbPhotosRenamed, nbPhotosRotated, nbVideosRenamed, totalTime);
    log.debug("");
  }
  ...
  private File renameFile(final File f, final Date originalDate) throws CustomException {
    try {
      ...
      String suffixStr =
          getSuffix().replaceAll(PhotoRenamerConstants.SUFFIX_ORIGINAL_FILENAME_TOKEN, oldFilenameWithoutExtension) + extension;
