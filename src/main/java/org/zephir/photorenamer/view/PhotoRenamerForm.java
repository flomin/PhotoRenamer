...
public class PhotoRenamerForm implements PhotoRenamerConstants {
  ...
  private void loadPreferences() {
    try {
      if (new File("PhotoRenamer.properties").exists()) {
        ...
        try (FileInputStream in = new FileInputStream("PhotoRenamer.properties")) {
          props.load(in);
        }
  ...
  private void savePreferences() {
    try {
      ...
      try (FileOutputStream out = new FileOutputStream("PhotoRenamer.properties")) {
  ...
  private void createSShell() {
    ...
    buttonInputFolder.addListener(SWT.Selection, event -> {
    ...
    try {
      ...
      new Thread(() -> {
        try {
          ...
          // processing finished
          Display.getDefault().syncExec(() -> buttonProceed.setEnabled(true));
        }
      }).start();
