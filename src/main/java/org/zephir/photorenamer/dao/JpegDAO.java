      ...
      if (metadata instanceof JpegImageMetadata) {
        ...
        if (outputSet != null) {
          ...
          if (removeFieldIfExists || outputSet.findField(ExifTagConstants.EXIF_TAG_DATE_TIME_ORIGINAL) == null) {
            TiffOutputField dateTimeOriginalField = new TiffOutputField(ExifTagConstants.EXIF_TAG_DATE_TIME_ORIGINAL,
                FieldType.FIELD_TYPE_ASCII, asciiTiffDate.length(), asciiTiffDate.getBytes());
